...
import RegistrarCanjeModal from '../pages/pharmacy/canjes/registrarCanjeModal';

const PharmacyNavbar: React.FC = () => {
  const [activeDropdown, setActiveDropdown] = useState<string | null>(null);
  //const [showModal, setShowModal] = useState<boolean>(false);
  const [showModalCanje, setShowModalCanje] = useState<boolean>(false);
  ...
  return (
    <nav className="navbar">
      <ul className="navbar-menu">
        <li className="navbar-item" onClick={() => toggleDropdown('usuarios')}>
          ...
          {activeDropdown === 'usuarios' && (
            <ul className="dropdown-menu">
              <li onClick={() => navigate('/users')}>Ver Usuarios</li> {/* Cambiar a Ver Usuarios */}
        ...
        <li className="navbar-item" onClick={() => toggleDropdown('canjes')}>
          ...
          {activeDropdown === 'canjes' && (
            <ul className="dropdown-menu">
              <li onClick={() => setShowModalCanje(true)}>Crear Canjes</li>
            </ul>
          )}
        </li>
      ...
      {showModalCanje && <RegistrarCanjeModal pharmacy='FarmaValue' closeModal={() => setShowModalCanje(false)} />}
    </nav>
  );
};
